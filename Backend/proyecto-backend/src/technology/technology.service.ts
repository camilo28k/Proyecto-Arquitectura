...
export class TechnologyService extends PrismaClient implements OnModuleInit {
  ...
  create(dto: CreateTechnologyDto, user: any) {
  ...
  findAll() {
    return this.technology.findMany({
      orderBy: { createdAt: 'desc' },
    });
  }

  async findOne(id: string) {
    const campaign = await this.technology.findUnique({ where: { id } });
  ...
  async update(id: string, dto: UpdateTechnologyDto, user: any) {
    const campaign = await this.technology.findUnique({ where: { id } });
  ...
  async remove(id: string, user: any) {
    ...
    return this.technology.delete({ where: { id } });
  }
}